    ...
    function checkGpu() {
        ...
        if (software) {
            console.info('[gpu] Software rendering, no GPU behind WebGL (' + renderer + '). '
                + (heavy ? 'Running the animated background anyway; expect a low frame rate. Add ?lite to use the static version.'
                         : 'The animated background is off. Add ?heavy to force it on.'));
        }
